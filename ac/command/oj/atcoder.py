...
=======
    ...
    def get_statement_a(self, contest_id, f5=0):
        ...
        if f5:
            ...
            if res.status_code != 200:
                raise Exception(
                    f'status_code {res.status_code}: https://atcoder.jp/contests/{contest_id}/tasks_print?lang=ja')

        tree = lxml.html.fromstring(res.text)
        statementx = tree.xpath(
    ...
    def get_problems(self, contest_id, f5=0):
        ...
        if f5:
            ...
            if res.status_code != 200:
                raise Exception(
                    f'status_code {res.status_code}: https://atcoder.jp/contests/{contest_id}/submit')
    ...
    def get_start_time(self, contest_id):
        res = self.session.get(f'https://atcoder.jp/contests/{contest_id}')
        tree = lxml.html.fromstring(res.text)
        start_str = tree.xpath(
            '//*[@class="fixtime fixtime-full"]')[0].text_content()
        # start_str = '2020-6-27 2:37:00+0900'
        return datetime.strptime(start_str[:-5], '%Y-%m-%d %H:%M:%S')
    ...
    def download_testcases(self, problem):
        ...
        if res.status_code != 200:
            raise Exception(
                f'status_code {res.status_code}: https://atcoder.jp/contests/{problem[1]}/tasks/{problem[2]}?lang=ja')
        ...
        while len(tree.xpath(f'//h3[text()="入力例 {cnt+1}"]')):
            ...
            input_data = tree.xpath(
                f'//h3[text()="入力例 {cnt}"]')[0].getnext().text
